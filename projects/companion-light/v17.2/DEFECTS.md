...
| CAT-014 | PERF | Librarian.swift · search(); Vault.swift · ftsCandidates()/recentCandidates() | allObjects() loads every embedding per query, O(N) | Search must touch a candidate pool, not the corpus | Fable | PATCHED — FTS5 (200) ∪ recent (100) → fetch pool only. Gap recorded: semantic-only hits outside the pool are invisible until sqlite-vec | OPEN |
...
| CAT-025 | CODE | ios/…/CompanionRuntimes/MLXRuntime.swift | VERIFY stub replaced | Adapters are written against source, not memory | Fable | PATCHED — against ml-explore/mlx-swift-lm@e3d4a20: LLMModelFactory.loadContainer(from:using:), ChatSession, GenerateParameters; EmbedderModelFactory + perform() for EmbeddingGemma. Not compiled. | — |
